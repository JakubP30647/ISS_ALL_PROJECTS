    ...
    try:
        while True:
            ...
            if cmd == "sterowanie":
                ...
                while True:
                    pokaz_menu(MENU_PID_AND_FUZZY, "PROJEKT 4")
                    try: iss4_choice = int(input("Wybierz numer: "))
                    except ValueError:
                        print("Niepoprawny wybor")
                        continue

                    if iss4_choice not in MENU_PID_AND_FUZZY:
                        print("Niepoprawny numer komendy")
                        continue

                    iss4_desc, iss4_cmd = MENU_PID_AND_FUZZY[iss4_choice]
                    print(iss4_cmd)
                    if iss4_cmd == "back":
                        break

                    if arduinoOBJ.ser and arduinoOBJ.ser.is_open:
                        commandsHandler(iss4_cmd, arduinoOBJ.ser, arduinoOBJ)
                    else:
                        print("[Main] Port nieotwarty, komenda pominieta")
